...
const quantityEl = document.getElementById("quantity"); // Kvantiet för en produkt
...
function addToBasket(el, id, name, cost, image, notify = true) {
    if(quantityEl!==null){
        numOfItems = parseInt(quantityEl.value);
    }
    ...
    for (var i = 0; i < currentBasket.length; i++) {
        if (id == currentBasket[i].artId) {
            numOfItems = currentBasket[i].nums + parseInt(quantityEl.value);
